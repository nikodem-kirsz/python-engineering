...
async def produce(name: int, q: asyncio.Queue) -> None:
    ...
    # Synchronous loop for each single producer to 
...
async def consume(name: int, q: asyncio.Queue) -> None:
    while True:
        ...
        # Processing on the task is complete give it back to event loop
        q.task_done()
...
async def main(nprod: int, ncon: int):
    ...
    # cancel the consumer tasks, which would otherwise hang up and wait
    # endlessly for additional queue items to appear.
    for c in consumers:
        c.cancel()
